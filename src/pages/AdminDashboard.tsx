...
export function AdminDashboard() {
  ...
  // Handle Admin Login submission with Firestore verification
  const handleAdminLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const snapshot = await getDocs(adminConfigRef);

      if (snapshot.empty) {
        // Initial setup bootstrap: If database is brand new, accept admin / admin2027 or seed entered credentials
        if (
          (cleanUser.toLowerCase() === 'admin' && cleanPass === 'admin2027') ||
          (cleanUser.length >= 3 && cleanPass.length >= 4)
        ) {
          try {
            await setDoc(doc(db, 'admin_config', 'primary_admin'), {
              username: cleanUser,
              password: cleanPass,
              role: 'Administrator',
            ...
            console.warn('Initial admin_config record auto-creation notice:', initErr);
          }
          sessionStorage.setItem('admin_authenticated', 'true');
          setIsAdminLoggedIn(true);
          setToastMessage('Welcome to Phyllis Wangui Campaign Console');
          return;
        } else {
          setLoginError('Initial setup: Default credentials are "admin" / "admin2027" or provide admin credentials.');
          setIsLoggingIn(false);
          return;
        }
      }

      // Check against Firestore admin_config collection documents
      let matched = false;
      snapshot.forEach((docSnap) => {
        ...
        if (
          data.username &&
          data.password &&
          data.username.trim().toLowerCase() === cleanUser.toLowerCase() &&
      ...
      // Fallback check if default admin credentials
      if (!matched && cleanUser.toLowerCase() === 'admin' && cleanPass === 'admin2027') {
        matched = true;
      }
      ...
      if (matched) {
        ...
        setLoginError('Invalid admin credentials. Please verify your username and password.');
      }
    } catch (err: any) {
      console.error('Error authenticating admin against Firestore:', err);
      // Fallback in case of network or permissions restriction
      if (cleanUser.toLowerCase() === 'admin' && cleanPass === 'admin2027') {
        sessionStorage.setItem('admin_authenticated', 'true');
        setIsAdminLoggedIn(true);
        setToastMessage('Authenticated via secure admin credentials');
      } else {
